---
title: "Cell types annotation with SingleR using a fine resolution reference for sc-/sn-RNA-Seq Analysis in 10X Flex Genomics data"
...
results_dir <- file.path(module_results_dir, "02_cell_types_annotation_SingleR_fine")
if (!dir.exists(results_dir)) {
  dir.create(results_dir)}

# Create plots directory
module_plots_dir <- file.path(analysis_dir, "plots") 
plots_dir <- file.path(module_plots_dir, "02_cell_types_annotation_SingleR_fine") 
...
```{r identify-cell-types-fine, echo=TRUE}
# label.fine - Fine cell type annotation
seurat_obj_fine <- SingleR(test = seurat_obj@assays[[assay]]@data, ref = bpe, labels = bpe$label.fine)

# Add metadata
new.data <- seurat_obj_fine %>%
  as.data.frame() %>%
  add_column(project = "project")
seurat_obj <- AddMetaData(seurat_obj, metadata = new.data)
seurat_obj@meta.data$singler.fine <- seurat_obj_fine$pruned.labels
#head(seurat_obj@meta.data)

# Create table
summary_table_pruned <- table(cell_type_fine = seurat_obj@meta.data$singler.fine) %>% #pruned labels
  as.data.frame() %>% 
  mutate(cells_number_pruned = Freq) %>% 
  dplyr::select(-Freq)

summary_table <- table(cell_type_fine = seurat_obj@meta.data$labels) %>% # all labels
  as.data.frame() %>% 
...
We will check and compare the pruned labels and the initial labels by summarizing the identified fine cell type annotations.

```{r, fig.align = "left", results = "asis", message = FALSE, warning = FALSE, echo = FALSE}
tables1 <- summary_table
cat("  \n<div align=\"center\" style=\"font-size:80%\">  \n")
print(knitr::kable(tables1, align = "lcccc", caption = "Fine cell type annotations before vs after label pruning"))
cat("  \n</div>  \n")
cat("  \n\\pagebreak  \n")
```
...
```{r plotScoreHeatmap-fine, fig.width = 15, fig.height = 15, fig.fullwidth = TRUE, echo=TRUE}
print(plotScoreHeatmap(seurat_obj_fine))
```
...
# Create table
summary_table <- table(cell_type_fine = seurat_obj@meta.data$labels, Removed = to.remove) %>%
  as.data.frame() %>% 
  pivot_wider(names_from = "Removed", values_from = "Freq") 
```

We will summarize the identified fine cell type annotations to be removed.

```{r, fig.align = "left", results = "asis", message = FALSE, warning = FALSE, echo = FALSE}
tables1 <- summary_table
cat("  \n<div align=\"center\" style=\"font-size:80%\">  \n")
print(knitr::kable(tables1, align = "lcccc", caption = "Fine cell type annotations to be removed"))
cat("  \n</div>  \n")
cat("  \n\\pagebreak  \n")
```
...
```{r plotDeltaDistribution-fine, fig.width = 15, fig.height = 15, fig.fullwidth = TRUE, echo=TRUE}
name <- paste0(plots_dir, "/", glue::glue("plotDeltaDistribution_fine.png"))
print(plotDeltaDistribution(seurat_obj_fine, ncol = 3))
...
seurat_obj@meta.data <- seurat_obj@meta.data %>%
  mutate(singler.fine = if_else(is.na(singler.fine), "unassigned", as.character(singler.fine)),
         singler.fine = if_else(singler.fine == "", "unassigned", singler.fine))

# unassigned cells 
cells_unassigned <- length(which(seurat_obj@meta.data$singler.fine == "unassigned"))
```
...
## Fine cell type annotations: before vs after integration

```{r plot-singler-comparison-before-vs-after-integration, fig.width = 35, fig.height = 12, fig.fullwidth = TRUE, echo=TRUE}
name <- paste0(plots_dir, "/", glue::glue("plot-singler-comparison-fine-before-vs-after-integration.png"))
p1.before <- DimPlot(seurat_obj, group.by = 'singler.fine', label = TRUE, repel = TRUE, label.size = 2.5, cols = palette) + ggtitle("Fine cell type annotations: merged") 
p1.after <- DimPlot(seurat_obj, reduction = reduction_value, group.by = 'singler.fine', label = TRUE, repel = TRUE, label.size = 2.5, cols = palette) + ggtitle(glue::glue("Fine cell type annotations: {integration_method} integration")) 
print(p1.before + p1.after)
ggsave(file = name, width = 35, height = 12, device = "png")
```

```{r plot-cell-types-project-fractions, fig.width = 6, fig.height = 5, fig.fullwidth = TRUE, echo=TRUE}
name <- paste0(plots_dir, "/", glue::glue("plot-singler-fine-project-fractions.png"))
p.project <- print(cell_type_fractions_fine(df = seurat_obj@meta.data,
...
## Fine cell type annotations and fractions per ID
...
name <- paste0(plots_dir, "/", glue::glue("plot-singler-fine-ID.png"))
p1.after <- print(DimPlot(seurat_obj, reduction = reduction_value, split.by = "ID", group.by = 'singler.fine', label = TRUE, repel = TRUE, label.size = 2.5, cols = palette) + 
                    ggtitle(glue::glue("Fine cell type annotations: {integration_method} integration")) +
...
name <- paste0(plots_dir, "/", glue::glue("plot-singler-fine-ID-fractions.png"))
p.ID <- print(cell_type_fractions_fine(df = seurat_obj@meta.data,
                                       condition_plot = "ID",
                                       color_df = palette,
                                       title_value = "Fine"))
ggsave(file = name, width = 6, height = 5, device = "png")
...
## Fine cell type annotations and fractions per `r condition_value1`
...
name <- paste0(plots_dir, "/", glue::glue("plot-singler-fine-condition1.png"))
p1.after <- print(DimPlot(seurat_obj, reduction = reduction_value, split.by = condition_value1, group.by = 'singler.fine', label = TRUE, repel = TRUE, label.size = 2.5, cols = palette) + 
                    ggtitle(glue::glue("Fine cell type annotations: {integration_method} integration")) +
...
name <- paste0(plots_dir, "/", glue::glue("plot-singler-fine-condition-fractions1.png"))
p.condition <- print(cell_type_fractions_fine(df = seurat_obj@meta.data,
...
## Fine cell type annotations and fractions per `r condition_value2`
...
if (!is.null(condition_value2)) {
  ...
  name <- paste0(plots_dir, "/", glue::glue("plot-singler-fine-condition2.png"))
  p1.after <- print(DimPlot(seurat_obj, reduction = reduction_value, split.by = condition_value2, group.by = 'singler.fine', label = TRUE, repel = TRUE, label.size = 2.5, cols = palette) + 
                    ggtitle(glue::glue("Fine cell type annotations: {integration_method} integration")) +
...
if (!is.null(condition_value2)) {
  name <- paste0(plots_dir, "/", glue::glue("plot-singler-fine-condition-fractions2.png"))
  p.condition <- print(cell_type_fractions_fine(df = seurat_obj@meta.data,
...
## Fine cell type annotations and fractions per `r condition_value3`
...
if (!is.null(condition_value3)) {
  ...
  name <- paste0(plots_dir, "/", glue::glue("plot-singler-fine-condition3.png"))
  p1.after <- print(DimPlot(seurat_obj, reduction = reduction_value, split.by = condition_value3, group.by = 'singler.fine', label = TRUE, repel = TRUE, label.size = 2.5, cols = palette) + 
                    ggtitle(glue::glue("Fine cell type annotations: {integration_method} integration")) +
...
if (!is.null(condition_value3)) {
  name <- paste0(plots_dir, "/", glue::glue("plot-singler-fine-condition-fractions3.png"))
  p.condition <- print(cell_type_fractions_fine(df = seurat_obj@meta.data,
...
#saveRDS(seurat_obj, file = paste0(results_dir, "/", "seurat_obj_SingleR_fine.rds"))
```
